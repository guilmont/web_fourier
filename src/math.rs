...
pub fn low_pass_matrix(inp_vec: &[f32], cutoff: usize) -> Result<Vec<Vec<f32>>, String> {
    let total_size = inp_vec.len();
    if total_size == 0 { return Err("Input vector is empty".into()); }
    ...
    // Reconstruct signal using only low frequencies
    let mut matrix = vec![vec![0.0f32; cutoff + 1]; total_size];
    for n in 0..total_size {
        matrix[n][0] = mean;
        for k in 1..=cutoff {
            let exp_term = Complex32::new(0.0, coeff * (n as f32) * (k as f32)).exp();
            matrix[n][k] = 2.0 * (trans[k] * exp_term).re;
        }
    }
